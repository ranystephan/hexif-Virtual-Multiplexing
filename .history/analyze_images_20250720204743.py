...
def analyze_single_image(image_path: str) -> Dict:
    ...
    try:
        ...
        # Get spatial dimensions (handle different image formats)
        if img.ndim == 3:
            # Check if it's RGB format (H, W, C) or multi-channel format (C, H, W)
            if img.shape[2] <= 4:  # Likely RGB/RGBA format (H, W, C)
                height, width = img.shape[0], img.shape[1]
            else:  # Likely multi-channel format (C, H, W)
                height, width = img.shape[1], img.shape[2]
        else:
            # For 2D images, spatial dimensions are [0] and [1]
            height, width = img.shape[0], img.shape[1]
        
        # Check minimum size using spatial dimensions
        min_size = 100
        if height < min_size or width < min_size:
            analysis['issues'].append(f"Too small: spatial size {height}x{width}, minimum: {min_size}x{min_size}")
            analysis['valid'] = False
        
        # Check aspect ratio using spatial dimensions
        ...
        # Analyze intensity
        if img.ndim == 3:
            # Check format and extract appropriate data for analysis
            if img.shape[2] <= 4:  # RGB/RGBA format (H, W, C)
                img_2d = np.mean(img, axis=2)  # Average across color channels
                analysis['num_channels'] = img.shape[2]
            else:  # Multi-channel format (C, H, W)
                img_2d = img[0]  # Use first channel
                analysis['num_channels'] = img.shape[0]
        elif img.ndim == 2:
            img_2d = img
...
def analyze_image_pairs(input_dir: str, he_suffix: str = "_HE.tif", orion_suffix: str = "_Orion.tif") -> pd.DataFrame:
    ...
    for he_file in he_files:
        ...
        if 'shape' in he_analysis:
            ...
            pair_result['he_channels'] = he_analysis.get('num_channels', 'unknown')
        ...
        if 'shape' in orion_analysis:
            ...
            pair_result['orion_channels'] = orion_analysis.get('num_channels', 'unknown')
