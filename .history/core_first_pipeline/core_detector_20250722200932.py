...
# Add import for the new TMA grid detector
...
class CoreDetectionConfig:
    ...
    max_detection_dimension: int = 2048  # Maximum dimension for detection processing
...
class CoreDetector:
    ...
    def detect_cores(self, image_path: str, image_type: str = "auto") -> Dict:
        ...
        # Store current image info for TMA grid detection
        self._current_image_path = image_path
        self._current_image_type = image_type
        
        # Memory-efficient loading: Load minimal data for image analysis first
        try:
            # Read image metadata first to determine type and size
            from tifffile import TiffFile
            with TiffFile(image_path) as tif:
                original_shape = tif.series[0].shape
                dtype = tif.series[0].dtype
                
            logger.info(f"Image metadata - shape: {original_shape}, dtype: {dtype}")
            
            # Determine image type if auto
            if image_type == "auto":
                image_type = self._determine_image_type_from_path(image_path, original_shape)
                self._current_image_type = image_type
            
            # For TMA grid detection, we don't need to load the detection image here
            if self.config.detection_method == "tma_grid":
                # Use TMA grid detection directly
                from .tma_grid_detector import TMAGridDetector, TMAGridConfig
                
                grid_config = TMAGridConfig(
                    expected_core_diameter_range=(800, 1200),
                    expected_spacing_range=(1000, 1600),
                    grid_detection_downsample=8,
                    template_match_threshold=0.3,
                    enable_visualizations=self.config.create_visualizations
                )
                
                grid_detector = TMAGridDetector(grid_config)
                grid_results = grid_detector.detect_tma_grid(image_path, image_type)
                ...
                if grid_results['success']:
                    ...
                    results = {
                        ...
                        'detection_image_shape': None,  # Not applicable for grid detection
                        'downsample_factor': grid_results.get('downsample_factor', 1.0),
                        'detection_processed_shape': None,
                        'total_cores_detected': len(grid_results['cores']),
                        'filtered_cores_count': len(grid_results['cores']),
                        'cores': grid_results['cores'],
                        'detection_method': 'tma_grid',
                        'config': self.config.__dict__,
                        'grid_parameters': grid_results.get('grid_parameters')
                    }
                    
                    logger.info(f"TMA grid detection completed: {len(grid_results['cores'])} cores")
                    return results
                else:
                    logger.warning("TMA grid detection failed, falling back to hybrid method")
                    # Continue with regular detection below
            
            # Load only detection channel efficiently for non-grid methods
            detection_image = self._load_detection_channel(image_path, image_type, original_shape)
            self._current_detection_image = detection_image
            
            logger.info(f"Image type: {image_type}, detection image shape: {detection_image.shape}")
            
        except Exception as e:
            logger.error(f"Failed to load image efficiently, falling back to full load: {e}")
        ...
        # Filter and validate cores using full-resolution parameters (but adjusted for scaling)
        filtered_cores = self._filter_cores_with_scaling(cores, detection_image, downsample_factor)
    ...
    def _determine_image_type_from_path(self, image_path: str, shape: Tuple) -> str:
        """Determine image type from path and shape without loading full image."""
        path_lower = str(image_path).lower()
        ...
        if len(shape) == 3:
            if shape[0] > 10:  # Many channels, likely multi-channel fluorescence
                return 'orion'
            elif shape[2] == 3:  # RGB, likely H&E
                return 'he'
        
        # Default to H&E for 2D images
        return 'he'

    def _load_detection_channel(self, image_path: str, image_type: str, original_shape: Tuple) -> np.ndarray:
        """Memory-efficiently load only the channel needed for detection."""
        
        if image_type == 'orion' and len(original_shape) == 3 and original_shape[0] > 10:
            # Multi-channel Orion - load only first channel (DAPI)
            logger.info("Loading only channel 0 (DAPI) for Orion detection to save memory")
            from tifffile import imread
            # Load only the first channel
            detection_img = imread(image_path, key=0)  # This loads only the first page/channel
            if detection_img.ndim == 3:
                detection_img = detection_img[0] if detection_img.shape[0] < detection_img.shape[2] else detection_img[:,:,0]
                
        elif image_type == 'he':
            # For H&E, we need RGB data, but we can downsample during loading if very large
            if max(original_shape[-2:]) > 50000:  # Very large image
                logger.info("Large H&E image detected, loading at reduced resolution")
                # Load with subsampling to reduce memory usage
                image = imread(image_path)
                if image.ndim == 3 and image.shape[2] == 3:
                    ...
                    detection_img = image[0] if image.ndim == 3 and image.shape[0] < image.shape[2] else image
            else:
                # Normal size H&E
                image = imread(image_path)
                detection_img = self._prepare_detection_image(image, image_type)
        else:
    ...
    def _detect_cores_with_method(self, image: np.ndarray, method: str) -> List[Dict]:
        ...
        elif method == "tma_grid":
            return self._detect_cores_tma_grid()
        else:
    ...
    def _detect_cores_morphology(self, image: np.ndarray) -> List[Dict]:
        ...
        # Apply morphological operations (scale-aware)
        # Adjust structuring element size based on image size to maintain effectiveness
        base_disk_size = max(1, self.config.morphology_iterations * 3)
        if max(binary.shape) < 3000:  # For downsampled images, use smaller elements
            disk_size = max(1, base_disk_size // 2)
        else:
        ...
        if distance.max() == 0:
            logger.warning("No foreground regions found in morphology detection")
            return []
        ...
        # Check if we found any local maxima
        if len(local_maxima) == 0:
            logger.warning("No local maxima found in distance transform")
            return []
    ...
    def _detect_cores_hybrid(self, image: np.ndarray) -> List[Dict]:
        ...
        if not all_cores:
            logger.warning("No cores detected by any method")
            return []
        ...
        try:
            ...
            logger.warning("sklearn not available, using simple distance-based deduplication")
            # Fallback: simple distance-based deduplication
            unique_cores = []
            used_indices = set()
            ...
            for i, core in enumerate(all_cores):
                ...
                pos_i = core['centroid_xy']
                unique_cores.append(core)
                used_indices.add(i)
                
                # Mark nearby cores as used
                for j in range(i + 1, len(all_cores)):
                    if j in used_indices:
                        continue
                    pos_j = all_cores[j]['centroid_xy']
                    distance = np.sqrt((pos_i[0] - pos_j[0])**2 + (pos_i[1] - pos_j[1])**2)
                    if distance < self.config.min_core_diameter * 0.3:
                        used_indices.add(j)
            
            logger.info(f"After simple deduplication: {len(unique_cores)} unique cores")
            return unique_cores
    
    def _detect_cores_tma_grid(self) -> List[Dict]:
        """Detect cores using TMA grid-based approach."""
        
        logger.info("Using TMA grid-based detection method")
        
        # Create TMA grid detector configuration
        grid_config = TMAGridConfig(
            expected_core_diameter_range=(800, 1200),
            expected_spacing_range=(1000, 1600),
            grid_detection_downsample=8,
        ...
        # We need the image path for grid detection
        # This is a bit of a hack - we'll store it during detect_cores
        if hasattr(self, '_current_image_path'):
            grid_results = grid_detector.detect_tma_grid(
                self._current_image_path, 
                getattr(self, '_current_image_type', 'auto')
            )
            
            if grid_results['success']:
                logger.info(f"TMA grid detection found {len(grid_results['cores'])} cores")
                return grid_results['cores']
            else:
                logger.warning("TMA grid detection failed, falling back to hybrid method")
                return self._detect_cores_hybrid(self._current_detection_image)
        else:
            logger.error("Image path not available for TMA grid detection")
            return []
    ...
    def _filter_cores_with_scaling(self, cores: List[Dict], detection_image: np.ndarray, downsample_factor: float) -> List[Dict]:
        """Filter detected cores with awareness of downsampling factor."""
        
        filtered_cores = []
        
        # Adjust filter parameters based on downsampling
        scale_factor = 1.0 / downsample_factor if downsample_factor != 1.0 else 1.0
        
        # More lenient filtering for highly downsampled images
        if downsample_factor < 0.1:  # Very aggressive downsampling
            area_tolerance = 0.3  # Allow 70% smaller than minimum
            circularity_tolerance = 0.5  # More lenient circularity
        elif downsample_factor < 0.5:  # Moderate downsampling
            area_tolerance = 0.1  # Allow 90% of minimum
            circularity_tolerance = 0.2  # Slightly more lenient
        else:
            area_tolerance = 0.0  # Standard filtering
            circularity_tolerance = 0.0
        
        adjusted_min_area = self.config.min_core_area * (1 - area_tolerance)
        adjusted_min_circularity = self.config.min_circularity * (1 - circularity_tolerance)
        
        logger.info(f"Filtering with adjusted parameters - min_area: {adjusted_min_area:.0f}, min_circularity: {adjusted_min_circularity:.3f}")
        
        for core in cores:
            # Size filters (adjusted)
            if not (adjusted_min_area <= core['area'] <= self.config.max_core_area):
                logger.debug(f"Core {core.get('id', '?')} filtered by area: {core['area']:.0f}")
                continue
            
            if core['equiv_diameter'] < self.config.min_core_diameter * (1 - area_tolerance):
                logger.debug(f"Core {core.get('id', '?')} filtered by diameter: {core['equiv_diameter']:.0f}")
                continue
            
            # Shape filters (adjusted)
            if core['circularity'] < adjusted_min_circularity:
                logger.debug(f"Core {core.get('id', '?')} filtered by circularity: {core['circularity']:.3f}")
                continue
            
            # More lenient solidity for downsampled images
            min_solidity = max(0.4, self.config.min_solidity * (1 - circularity_tolerance))
            if core.get('solidity', 1.0) < min_solidity:
                logger.debug(f"Core {core.get('id', '?')} filtered by solidity: {core.get('solidity', 1.0):.3f}")
                continue
            
            # More lenient aspect ratio
            max_aspect_ratio = self.config.aspect_ratio_threshold * (1 + circularity_tolerance)
            if core.get('aspect_ratio', 1.0) > max_aspect_ratio:
                logger.debug(f"Core {core.get('id', '?')} filtered by aspect ratio: {core.get('aspect_ratio', 1.0):.3f}")
                continue
            
            # Intensity filters (more lenient for downsampled)
            min_intensity = self.config.min_mean_intensity * (1 - area_tolerance)
            if core['mean_intensity'] < min_intensity:
                logger.debug(f"Core {core.get('id', '?')} filtered by intensity: {core['mean_intensity']:.1f}")
                continue
        ...
        logger.info(f"Filtering results: {len(filtered_cores)}/{len(cores)} cores passed filters")
        return filtered_cores

    def _filter_cores(self, cores: List[Dict], image: np.ndarray) -> List[Dict]:
        """Filter detected cores based on quality criteria (original method for compatibility)."""
        return self._filter_cores_with_scaling(cores, image, 1.0)
