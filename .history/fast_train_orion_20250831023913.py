...
@torch.no_grad()
def save_sample_grid(model, val_loader, device, out_png: Path, gamma: float = 0.85):
    """Save a simple comparison grid: H&E, Orion max-proj, then per-channel GT/Pred."""
    model.eval()
    ...
    for he, ori in val_loader:
        ...
        gt_np = ori[0].float().cpu().numpy()  # (20,H,W)
        pr_np = pred[0].float().cpu().numpy()
        break

    # Robust normalize helpers
    def rnorm(x):
    ...
    for c in range(min(3, he_disp.shape[2])):
        he_disp[..., c] = rnorm(he_disp[..., c])
    max_proj = rnorm(gt_np.max(axis=0))

    rows, cols = 21, 2
    fig_h = 3 + 20 * 1.6
    fig, ax = plt.subplots(rows, cols, figsize=(9, fig_h))
    ax[0, 0].imshow(he_disp)
    ax[0, 0].set_title("H&E")
    ax[0, 0].axis('off')
    ax[0, 1].imshow(max_proj**gamma, cmap='gray')
    ax[0, 1].set_title("Orion max-proj")
    ax[0, 1].axis('off')
    for i in range(min(20, gt_np.shape[0])):
        r = i + 1
        gt_ch = rnorm(gt_np[i])**gamma
        pr_ch = rnorm(pr_np[i])**gamma
        ax[r, 0].imshow(gt_ch, cmap='magma', vmin=0, vmax=1)
        ax[r, 0].set_title(f"Ch {i} — GT", fontsize=9)
        ax[r, 0].axis('off')
        ax[r, 1].imshow(pr_ch, cmap='magma', vmin=0, vmax=1)
        ax[r, 1].set_title(f"Ch {i} — Pred", fontsize=9)
        ax[r, 1].axis('off')
    plt.tight_layout()
...
def make_loaders(args, device):
    ...
    val_ds = OrionPairsDataset(
        ...
        sampling="grid", grid_stride=args.grid_stride, augment=False,
    )
    ...
    logging.info(f"Dataset size: {len(train_ds)} patches, using {args.num_workers} workers (pin_memory={pin})")
...
def main():
    ...
    p.add_argument("--save_every", type=int, default=10, help="save checkpoint and sample PNG every N epochs")
    args = p.parse_args()
    ...
    # Persist config (include model_class for downstream loading)
    cfg = dict(vars(args))
    cfg["model_class"] = "LightweightUNet"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Periodic checkpoint and sample predictions
        if (epoch % args.save_every == 0) or (epoch == args.epochs):
            ...
            try:
                save_sample_grid(model=model, val_loader=val_loader, device=device, out_png=png_path, gamma=0.85)
