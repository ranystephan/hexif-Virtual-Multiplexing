...
The source data consists of paired whole-slide images: one stained with H\&E and the other imaged using the 20-channel Orion platform. Both slides originate from the same tissue microarray (TMA), providing hundreds of distinct tissue cores. The Orion panel contains markers targeting immune and stromal cell populations, including IBA1 (pan-macrophage), CD163 (broad macrophage), CD3e (T-cell), Pan-CK (epithelial/cancer cells), SPP1, HLA-DR, FAP, CD8a, FOLR2, GFPT2, NLRP3, FOXP3, CD15, LYVE1, SMA, and IL-4I1, with particular focus on characterizing macrophage subsets and immune cell interactions.
...
\noindent
The network takes a 3-channel H\&E image patch as input. As an optional feature, boundary guidance can be enabled by computing a cell boundary map from the H\&E patch using a Sobel edge detector applied to a Gaussian-filtered grayscale version of the input. When enabled, this single-channel boundary map is concatenated with the RGB input, creating a 4-channel input tensor. The model outputs a 20-channel tensor of the same height and width as the input, with a final sigmoid activation to ensure predicted fluorescence intensities are in the range $[0, 1]$.
...
\[ L_{\text{total}} = w_{\text{mse}} L_{\text{mse}} + w_{\text{l1}} L_{\text{l1}} + w_{\text{bound}} L_{\text{bound}} + w_{\text{var}} L_{\text{var}} \]
\begin{itemize}
    \item \textbf{Mean Squared Error Loss ($L_{\text{mse}}$):} Measures pixel-wise squared differences between predicted $\hat{Y}$ and ground truth $Y$:
    \[ L_{\text{mse}} = \text{MSE}(\hat{Y}, Y) \]
    
    \item \textbf{L1 Loss ($L_{\text{l1}}$):} Measures pixel-wise absolute differences, encouraging sparsity which is characteristic of fluorescence images:
    \[ L_{\text{l1}} = \text{MAE}(\hat{Y}, Y) \]

    \item \textbf{Boundary-Aware Loss ($L_{\text{bound}}$):} When boundary guidance is enabled, this term applies higher weight to reconstruction error at cell boundaries. Given a boundary mask $M_{\text{bound}}$ from the H\&E input:
    \[ L_{\text{bound}} = \text{MSE}(\hat{Y} \odot M_{\text{bound}}, Y \odot M_{\text{bound}}) \]
    where $\odot$ is the element-wise product.

    \item \textbf{Anti-Collapse Regularization ($L_{\text{var}}$):} To prevent mode collapse where the model produces uniform outputs, this term penalizes predictions with insufficient spatial variance. For predictions with spatial variance below threshold $\tau_v = 0.01$:
    \[ L_{\text{var}} = (\tau_v - \text{Var}_{\text{spatial}}(\hat{Y}))^2 \]
    where $\text{Var}_{\text{spatial}}$ is the mean variance across spatial dimensions for each channel.
\end{itemize}

\subsubsection{Training Details}
The model was implemented in PyTorch and trained using the AdamW optimizer with a OneCycle learning rate schedule, which starts with a low learning rate, warms up to a maximum value, and then anneals down. We used a core-level split to create independent training and validation sets, ensuring that all patches from a single tissue core belong to only one set, preventing data leakage. Training was performed with automatic mixed precision and gradient clipping (max norm = 1.0) for stability. Early stopping with patience monitoring was employed to prevent overfitting.
