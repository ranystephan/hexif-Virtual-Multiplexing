...
def load_channel_statistics(stats_path: Optional[str], C: int, primary_threshold: float,
    ...
    fallback_key = None
    for key in next(iter(aggregated.values())).keys():
        if key.startswith("pos_fraction_") and key.endswith("_mean"):
            fallback_key = key
            if key == coverage_key:
                break
    if coverage_key not in next(iter(aggregated.values())).keys():
