...
def convert_ome_tiff(input_path: str, output_path: str, max_resolution: int = None) -> bool:
    ...
    try:
        ...
        if processed_img.ndim == 3 and processed_img.shape[0] <= 50:
            # Multi-channel format - save with proper axes metadata
            imwrite(
                output_path, 
                processed_img,
                photometric='minisblack',  # Prevent RGB interpretation
                metadata={'axes': 'CYX'} if processed_img.shape[0] > 1 else None,
                compression='lzw'
            )
        else:
            # RGB or grayscale
