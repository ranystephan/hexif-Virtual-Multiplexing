    ...
    try:
        ...
        try:
            if processed_img.ndim == 3 and processed_img.shape[0] <= 50 and processed_img.shape[0] != 3:
                # Multi-channel format (not RGB) - save with OME-TIFF metadata for VALIS
                logger.info(f"Saving as multi-channel OME-TIFF with {processed_img.shape[0]} channels")
                
                # Create proper OME-XML metadata
                from tifffile import OmeXml
                ome_xml = OmeXml()
                ome_xml.addimage(
                    dtype=processed_img.dtype,
                    shape=processed_img.shape,
                    storedshape=processed_img.shape,
