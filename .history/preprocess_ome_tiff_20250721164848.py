...
def convert_ome_tiff(input_path: str, output_path: str, max_resolution: int = None, registration_channel: int = None) -> bool:
    """
    ...
    try:
        ...
        auto_resize_threshold = 20000  # 20k pixels - more conservative for TIFF compatibility
        ...
        try:
            if processed_img.ndim == 3 and processed_img.shape[0] <= 50 and processed_img.shape[0] != 3:
                # Multi-channel format (not RGB) - save as simple multi-page TIFF
                logger.info(f"Saving as multi-channel TIFF with {processed_img.shape[0]} channels")
                
                # Save as multi-page TIFF (each channel as a separate page)
                imwrite(
                    output_path, 
                    processed_img,
                ...
                logger.info(f"Image size: {processed_img.shape[1]} x {processed_img.shape[2]} pixels")
                
                # Handle very large images more carefully
                try:
                    processed_img_rgb = np.transpose(processed_img, (1, 2, 0))
                    logger.info(f"Transposed to shape: {processed_img_rgb.shape}")
                except MemoryError:
                    logger.error("Not enough memory to transpose large image")
                    raise ValueError("Image too large to process - try using --max_preprocess_resolution to resize")
            ...
            logger.info("Trying fallback saving methods...")
            
            # Fallback 1: save without compression
            try:
                if processed_img.ndim == 3 and processed_img.shape[0] == 3:
                    # Try converting (C, H, W) to (H, W, C) for RGB
                    processed_img_rgb = np.transpose(processed_img, (1, 2, 0))
                    imwrite(output_path, processed_img_rgb)
                else:
                    imwrite(output_path, processed_img)
            except Exception as fallback_error:
                logger.warning(f"Fallback 1 failed: {fallback_error}")
                
                # Fallback 2: save as-is without any conversion
                try:
                    logger.info("Saving image in original format without conversion...")
                    imwrite(output_path, processed_img.astype(np.uint8))
                except Exception as final_error:
                    logger.error(f"All saving methods failed: {final_error}")
                    raise ValueError(f"Unable to save processed image: {final_error}")
