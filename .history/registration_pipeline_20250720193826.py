...
class RegistrationPipeline:
    ...
    def _find_image_pairs(self) -> List[Tuple[str, str, str]]:
        ...
        for he_file in he_files:
            # Extract core ID by removing the _HE.tif suffix more robustly
            filename = he_file.name
            if filename.endswith(self.config.he_suffix):
                # Remove the suffix from the end of the filename
                core_id = filename[:-len(self.config.he_suffix)]
            else:
                # Fallback: remove the suffix from stem
                core_id = he_file.stem.replace(self.config.he_suffix, "")
            ...
            if orion_file.exists():
                ...
                logger.debug(f"Looking for: {orion_file}")
    ...
    def _generate_summary(self, registration_results: List[Dict], quality_df: pd.DataFrame, pairs_dir: Optional[str]) -> Dict:
        ...
        summary = {
            ...
            'success_rate': successful_registrations / total_pairs if total_pairs > 0 else 0,
            'quality_metrics': {
                'mean_ssim': quality_df['ssim'].mean(),
                'mean_ncc': quality_df['ncc'].mean(),
                'mean_mutual_info': quality_df['mutual_info'].mean(),
                'passed_quality_thresholds': quality_df[['passed_ssim', 'passed_ncc', 'passed_mi']].all(axis=1).sum()
            },
