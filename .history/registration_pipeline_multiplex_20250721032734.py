    ...
    def register_core_pair(self, he_path: str, orion_path: str, core_id: str) -> Dict:
        ...
        try:
            ...
            try:
                ...
                # IMPORTANT: Load the FULL multi-channel Orion image
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                # Save images for VALIS (use DAPI channel for registration)
                he_file = src_dir / "he.png"
                orion_file = src_dir / "orion.png"
                full_orion_file = src_dir / "orion_full.tif"
                
                try:
                    skio.imsave(he_file, he_img, check_contrast=False)
                    skio.imsave(orion_file, dapi_img, check_contrast=False)
                    # Save full multi-channel Orion image for warping
                    imwrite(full_orion_file, full_orion_img)
                except Exception as e:
                ...
                try:
                    # Warp and save - this will warp both the DAPI and full Orion images
                    ...
                    # Load warped results - prioritize the full multi-channel version
                    warped_full_orion_path = out_dir / full_orion_file.with_suffix(".ome.tiff").name
                    warped_dapi_path = out_dir / orion_file.with_suffix(".ome.tiff").name
                    
                    warped_orion = None
                    
                    # Try to load the warped full multi-channel image first
                    if warped_full_orion_path.exists():
                        logger.info(f"Loading warped full Orion image from: {warped_full_orion_path}")
                        warped_orion = imread(warped_full_orion_path)
                        logger.info(f"Warped full Orion shape: {warped_orion.shape}")
                    
                    # Fallback to DAPI if full image not available
                    if warped_orion is None and warped_dapi_path.exists():
                        logger.warning("Full Orion warping failed, using DAPI channel only")
                        warped_orion = imread(warped_dapi_path)
                        logger.info(f"Warped DAPI shape: {warped_orion.shape}")
                    
                    if warped_orion is None or warped_orion.size == 0:
                        raise ValueError("VALIS failed to generate any warped output")
                ...
                try:
                    ...
                    if warped_orion.ndim == 3 and warped_orion.shape[0] > 1:
                        # Multi-channel image - use first channel (should be DAPI)
                        dapi_for_metrics = warped_orion[0] if warped_orion.shape[0] < warped_orion.shape[1] else warped_orion[:, :, 0]
                    else:
                        dapi_for_metrics = warped_orion if warped_orion.ndim == 2 else warped_orion[:, :, 0]
