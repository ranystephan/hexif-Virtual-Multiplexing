        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                try:
                    ...
                    # Save the FULL multi-channel Orion image as TIFF
                    # VALIS will automatically use the first/DAPI channel for registration
                    imwrite(orion_file, full_orion_img)
                    logger.info(f"Saved H&E image: {he_file} (shape: {he_img.shape})")
                    logger.info(f"Saved Orion image: {orion_file} (shape: {full_orion_img.shape})")
                except Exception as e:
                ...
                try:
                    ...
                    # VALIS will automatically handle the multi-channel Orion image
                    registrar = registration.Valis(
                        str(src_dir),
                        str(dst_dir),
                        reference_img_f=he_file.name,  # Use H&E as reference
                        image_type="multi",  # Tell VALIS this is mixed image types
