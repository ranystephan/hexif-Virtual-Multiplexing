...
try:
    from registration_pipeline import DatasetPreparator, RegistrationPipeline
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                try:
                    ...
                    if full_orion_img.ndim == 3:
                        # Check if it's already in (C, H, W) format
                        # Logic: if first dimension is much smaller than others, it's likely channels
                        if full_orion_img.shape[0] <= 100 and full_orion_img.shape[0] < min(full_orion_img.shape[1], full_orion_img.shape[2]):
                            # Already in (C, H, W) format - channels first
                            orion_for_valis = full_orion_img
                            logger.info(f"Orion image already in (C,H,W) format: {orion_for_valis.shape}")
                        else:
                            # Likely in (H, W, C) format - convert to (C, H, W)
                            orion_for_valis = np.transpose(full_orion_img, (2, 0, 1))
                            logger.info(f"Converted Orion from (H,W,C) to (C,H,W): {orion_for_valis.shape}")
                    else:
                        orion_for_valis = full_orion_img
                    
                    # Save with proper metadata to indicate it's multi-channel, not RGB
                    # Use simpler TIFF format that VALIS can handle better
                    imwrite(
                        orion_file, 
                        orion_for_valis,
                ...
                try:
                    ...
                    try:
                        ...
                        # Debug: Check what VALIS detected
                        for slide_name, slide_obj in registrar.slide_dict.items():
                            logger.info(f"VALIS detected slide '{slide_name}': {slide_obj.slide_dimensions_wh}, "
                                      f"channels={slide_obj.reader.metadata.n_channels}, "
                                      f"is_rgb={slide_obj.reader.metadata.is_rgb}, "
                                      f"img_type='{slide_obj.img_type}'")
                        
                    except Exception as reg_error:
                        # Log detailed error info
                        logger.error(f"VALIS registration step failed: {reg_error}")
                        
                        # Debug: Check what VALIS detected before failing
                        try:
                            for slide_name, slide_obj in registrar.slide_dict.items():
                                logger.info(f"VALIS detected slide '{slide_name}': {slide_obj.slide_dimensions_wh}, "
                                          f"channels={slide_obj.reader.metadata.n_channels}, "
                            ...
                            logger.warning("Could not extract slide detection info after registration failure")
                        
                        raise ValueError(f"VALIS registration failed during processing: {reg_error}")
                ...
                try:
                    ...
                    warped_he_path = out_dir / he_file.with_suffix(".ome.tiff").name
                    ...
                    # --- NEW: Save warped OME-TIFFs to output directory ---
                    import shutil
                    permanent_dir = pathlib.Path(self.config.output_dir) / "registered_images"
                    permanent_dir.mkdir(exist_ok=True)
                    # Copy warped Orion
                    permanent_orion_path = permanent_dir / warped_orion_path.name
                    shutil.copy2(warped_orion_path, permanent_orion_path)
                    logger.info(f"Saved warped Orion OME-TIFF to: {permanent_orion_path}")
                    # Copy warped H&E if it exists
                    if warped_he_path.exists():
                        permanent_he_path = permanent_dir / warped_he_path.name
                        shutil.copy2(warped_he_path, permanent_he_path)
                        logger.info(f"Saved warped H&E OME-TIFF to: {permanent_he_path}")
                    # --- END NEW ---
...
if not BASE_CLASSES_AVAILABLE:
    ...
    class QualityController:
        ...
        def create_quality_plots(self, registration_results: List[Dict], output_dir: str):
            """Create quality control plots and a multi-channel RGB visualization using valis.viz.color_multichannel."""
            try:
                ...
                if not successful_results:
                    logger.warning("No successful registrations to plot")
                    return
                
                # Create simple metric plots
                metrics = [r['metrics'] for r in successful_results]
                core_ids = [r['core_id'] for r in successful_results]
                
                fig, axes = plt.subplots(1, 3, figsize=(15, 5))
                
                # SSIM plot
                ssim_values = [m.get('ssim', 0.0) for m in metrics]
                axes[0].bar(core_ids, ssim_values)
                axes[0].set_title('SSIM Values')
                axes[0].set_ylabel('SSIM')
                axes[0].tick_params(axis='x', rotation=45)
                
                # NCC plot
                ncc_values = [m.get('ncc', 0.0) for m in metrics]
                axes[1].bar(core_ids, ncc_values)
                axes[1].set_title('Normalized Cross-Correlation')
                axes[1].set_ylabel('NCC')
                axes[1].tick_params(axis='x', rotation=45)
                
                # Mutual Information plot
                mi_values = [m.get('mutual_info', 0.0) for m in metrics]
                axes[2].bar(core_ids, mi_values)
                axes[2].set_title('Mutual Information')
                axes[2].set_ylabel('MI')
                axes[2].tick_params(axis='x', rotation=45)
                
                plt.tight_layout()
                output_path = os.path.join(output_dir, "registration_quality_multiplex.png")
                plt.savefig(output_path, dpi=150, bbox_inches='tight')
                plt.close()
                logger.info(f"Quality plots saved to: {output_path}")
                
                # Multi-channel visualization for the first successful registration
                try:
                    import valis.viz
                    import matplotlib.pyplot as plt
                    rgb_vis_path = os.path.join(output_dir, "warped_orion_rgb_visualization.png")
                    warped_orion = successful_results[0]['warped_orion']
                    # Use valis.viz.color_multichannel to create an RGB visualization
                    n_channels = warped_orion.shape[0] if warped_orion.shape[0] < warped_orion.shape[-1] else warped_orion.shape[-1]
                    # Generate marker colors
                    marker_colors = valis.viz.get_n_colors(np.array([[255,0,0],[0,255,0],[0,0,255],[255,255,0],[0,255,255],[255,0,255],[128,128,128],[255,128,0],[0,128,255],[128,0,255]]), n_channels)
                    # If (C, H, W), transpose to (H, W, C) for visualization
                    if warped_orion.shape[0] == n_channels:
                        orion_vis = np.transpose(warped_orion, (1,2,0))
                    else:
                        orion_vis = warped_orion
                    rgb_img = valis.viz.color_multichannel(orion_vis, marker_colors, rescale_channels=True)
                    plt.figure(figsize=(8,8))
                    plt.imshow(rgb_img)
                    plt.title('RGB Visualization of Warped Orion (valis.viz.color_multichannel)')
                    plt.axis('off')
                    plt.savefig(rgb_vis_path, dpi=150, bbox_inches='tight')
                    plt.close()
                    logger.info(f"Saved multi-channel RGB visualization to: {rgb_vis_path}")
                except Exception as viz_e:
                    logger.warning(f"Failed to create multi-channel RGB visualization: {viz_e}")
            except Exception as e:
                logger.warning(f"Failed to create quality plots: {e}")
