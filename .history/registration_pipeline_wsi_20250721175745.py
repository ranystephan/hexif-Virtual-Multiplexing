    ...
    def detect_cores(self, image: np.ndarray, image_name: str = "image") -> List[Dict]:
        ...
        if len(image.shape) == 3:
            if image.shape[2] == 3:  # RGB (H, W, C)
                ...
                logger.info(f"Detected (C, H, W) format with {image.shape[0]} channels, using channel 0 for core detection")
                gray = image[0, :, :]  # Take first channel (e.g., DAPI)
            else:
                # Assume (H, W, C) format - channels last
                logger.info(f"Detected (H, W, C) format, using first channel for core detection")
                gray = image[:, :, 0]
...
class WSIRegistrationPipeline:
    ...
    def run(self) -> Dict:
        ...
        try:
            ...
            if self.config.save_core_detection_plots:
                ...
                # Use RGB H&E for visualization if available, otherwise use registered grayscale
                he_for_viz = he_rgb if he_rgb is not None else he_registered
                self._create_visualizations(he_for_viz, orion_registered, he_cores, orion_cores, matched_pairs)
    ...
    def _register_wsi(self) -> Dict:
        ...
        try:
            ...
            logger.info(f"Copying H&E from {self.config.he_wsi_path} to {he_temp_path}")
            logger.info(f"Copying Orion from {self.config.orion_wsi_path} to {orion_temp_path}")
            
            if not he_temp_path.exists():
                if not pathlib.Path(self.config.he_wsi_path).exists():
                    raise FileNotFoundError(f"H&E preprocessed file not found: {self.config.he_wsi_path}")
                shutil.copy2(self.config.he_wsi_path, str(he_temp_path))
                
            if not orion_temp_path.exists():
                if not pathlib.Path(self.config.orion_wsi_path).exists():
                    raise FileNotFoundError(f"Orion preprocessed file not found: {self.config.orion_wsi_path}")
                shutil.copy2(self.config.orion_wsi_path, str(orion_temp_path))
            ...
            try:
                ...
                if "NoneType" in str(e) or "copy mode not allowed" in str(e):
                    ...
                    try:
                        ...
                        logger.warning("All registration methods failed, attempting identity transformation as last resort")
                        
                        # As a last resort, try to create a minimal registration with identity transformation
                        # This allows the pipeline to continue even if registration fails
                        try:
                            # Clean up any JVM issues
                            try:
                                registration.kill_jvm()
                            except:
                                pass
                            
                            # Create fake registration results
                            logger.warning("Using identity transformation - images will not be aligned but cores can still be extracted")
                            rigid_registrar = None
                            non_rigid_registrar = None
                            error_df = None
                            
                            # Copy original files as "registered" versions
                            he_src = src_dir / "he_slide.tif"
                            orion_src = src_dir / "orion_slide.tif"
                            he_dst = self.registered_wsi_dir / "he_slide.ome.tiff"
                            orion_dst = self.registered_wsi_dir / "orion_slide.ome.tiff"
                            
                            shutil.copy2(str(he_src), str(he_dst))
                            shutil.copy2(str(orion_src), str(orion_dst))
                            
                            logger.warning("Identity transformation applied - proceeding with unaligned images")
                            
                        except Exception as final_error:
                            logger.error(f"Even identity transformation failed: {final_error}")
                            raise ValueError(f"All registration attempts failed: {e}")
                else:
                    raise
            
            # Save registered slides (only if we have a valid registrar)
            if rigid_registrar is not None:
                ...
                logger.info("Skipping VALIS save (using identity transformation - files already copied)")
    ...
    def _load_registered_images(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Ensure spatial dimensions match for core detection
        he_spatial_dims = he_registered.shape[:2] if he_registered.ndim >= 2 else he_registered.shape
        
        if orion_registered.ndim == 3 and orion_registered.shape[0] <= 50:
            # Multi-channel format (C, H, W)
            orion_spatial_dims = orion_registered.shape[1:3]
        else:
            # Single-channel or (H, W) format
            orion_spatial_dims = orion_registered.shape[:2] if orion_registered.ndim >= 2 else orion_registered.shape
        
        # Check if spatial dimensions match
        if he_spatial_dims != orion_spatial_dims:
            logger.warning(f"Spatial dimension mismatch: H&E {he_spatial_dims} vs Orion {orion_spatial_dims}")
            logger.info("Resizing Orion to match H&E spatial dimensions")
            
            target_height, target_width = he_spatial_dims
            
            if orion_registered.ndim == 3 and orion_registered.shape[0] <= 50:
                # Multi-channel format (C, H, W) - resize each channel
                resized_channels = []
                for c in range(orion_registered.shape[0]):
                    resized_channel = cv2.resize(orion_registered[c], (target_width, target_height), 
                                               interpolation=cv2.INTER_AREA)
                    resized_channels.append(resized_channel)
                orion_registered = np.stack(resized_channels, axis=0)
                logger.info(f"Resized multi-channel Orion to: {orion_registered.shape}")
            else:
                # Single-channel format
                orion_registered = cv2.resize(orion_registered, (target_width, target_height), 
                                            interpolation=cv2.INTER_AREA)
                logger.info(f"Resized single-channel Orion to: {orion_registered.shape}")
        else:
            logger.info("Spatial dimensions match - no resizing needed")
