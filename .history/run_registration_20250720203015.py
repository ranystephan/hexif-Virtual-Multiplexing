...
def main():
    ...
    if args.config:
        ...
        config = RegistrationConfig(
            ...
            max_failures_before_stop=args.max_failures
        )
    ...
    # Analyze images first if requested
    if args.analyze_first:
        print("Analyzing images before registration...")
        try:
            from analyze_images import analyze_image_pairs, create_analysis_report
            df = analyze_image_pairs(config.input_dir, config.he_suffix, config.orion_suffix)
            summary = create_analysis_report(df, config.output_dir + "_analysis")
            
            valid_rate = summary['valid_pairs'] / summary['total_pairs']
            if valid_rate < 0.5:
                print(f"\nWarning: Only {valid_rate:.1%} of images are valid for registration")
                response = input("Continue anyway? (y/N): ")
                if response.lower() != 'y':
                    print("Aborting registration")
                    return
        except ImportError:
            print("Image analysis not available (missing dependencies)")
