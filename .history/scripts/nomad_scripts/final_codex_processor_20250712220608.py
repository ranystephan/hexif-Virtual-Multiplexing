#!/usr/bin/env python3
"""
Final CODEX Processing Script
=============================

This script completely avoids pandas DataFrame creation issues by using only CSV operations.
"""
...
class FinalCODEXProcessor:
    """
    Final CODEX processor that uses only CSV operations
    """
    ...
    def create_combined_matrix_csv(self) -> bool:
        """
        Create combined expression matrix using only CSV operations
        
        Returns:
            True if successful, False otherwise
        """
        logger.info("Creating combined expression matrix using CSV")
...
def main():
    ...
    processor = FinalCODEXProcessor(data_root, output_root)
    
    # Process all TMAs
    results = processor.process_all_tmas()
    
    # Create combined matrix using CSV only
    success = processor.create_combined_matrix_csv()
