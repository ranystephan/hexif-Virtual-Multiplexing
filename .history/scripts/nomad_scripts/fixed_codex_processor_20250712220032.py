...
class FixedCODEXProcessor:
    """
    Fixed CODEX processor that properly handles all data types
    """
    ...
    def _save_results(self, cell_features: np.ndarray, cell_locations: Optional[np.ndarray], 
        ...
        try:
            ...
            for i, marker in enumerate(markers_list):
                expression_dict[marker] = [float(row[i]) for row in cell_features_list]
            
            # Create DataFrame with explicit data and columns
            expression_df = pd.DataFrame(data=expression_dict)
            ...
            if cell_locations is not None and len(cell_locations) > 0:
                ...
                locations_dict = {
                    'x': [float(row[0]) for row in cell_locations_list],
                    'y': [float(row[1]) for row in cell_locations_list]
                }
                locations_df = pd.DataFrame(data=locations_dict)
    ...
    def create_combined_matrix(self) -> pd.DataFrame:
        ...
        if not all_expression_data:
            ...
            empty_data = {}
            for col in ['tma', 'tissue', 'cell_id'] + markers_list:
                empty_data[col] = []
...
def main():
    ...
    processor = FixedCODEXProcessor(data_root, output_root)
