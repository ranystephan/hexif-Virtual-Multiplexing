...
class SimpleCODEXProcessor:
    ...
    def _save_results(self, cell_features: np.ndarray, cell_locations: Optional[np.ndarray], 
        ...
        try:
            ...
            cell_features_list = cell_features.tolist() if hasattr(cell_features, 'tolist') else cell_features
            ...
            if cell_locations is not None and len(cell_locations) > 0:
                cell_locations_list = cell_locations.tolist() if hasattr(cell_locations, 'tolist') else cell_locations
    ...
    def create_combined_matrix(self) -> pd.DataFrame:
        ...
        if not all_expression_data:
            ...
            return pd.DataFrame(columns=['tma', 'tissue', 'cell_id'] + self.markers)
