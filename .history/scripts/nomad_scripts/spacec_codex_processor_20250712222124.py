...
class SPACECCODEXProcessor:
    ...
    def _process_tissue_extraction(self, qptiff_file: Path, output_dir: Path) -> List[Path]:
        ...
        try:
            ...
            # Step 2: Segment individual tissue pieces
            tissueframe = sp.tl.label_tissue(
                resized_im,
                lower_cutoff=0.2,  # Adjust based on your data
                upper_cutoff=0.21  # Adjust based on your data
            )
            
            logger.info(f"Identified {len(tissueframe['region'].unique())} tissue pieces")
            
            # Step 3: Extract individual labeled tissues
            extracted_tissues = []
            for region in tissueframe['region'].unique():
            ...
            logger.info(f"Extracted {len(extracted_tissues)} tissue pieces")
