#!/usr/bin/env python3
"""
Ultimate CODEX Processing Script
================================

This script completely avoids pandas DataFrame creation issues by using alternative methods.
"""
...
class UltimateCODEXProcessor:
    """
    Ultimate CODEX processor that avoids pandas DataFrame issues
    """
    ...
    def create_combined_matrix(self) -> pd.DataFrame:
        ...
        for expression_file in self.output_root.rglob("*_expression_matrix.csv"):
            try:
                # Read CSV directly without pandas
                data = []
                with open(expression_file, 'r') as csvfile:
                    ...
                    for row in reader:
                        if len(row) > 1:  # Ensure row has data
                            ...
                            expression_values = [float(val) for val in row[1:]]
                            
                            # Add metadata
                            row_data = {
                                'cell_id': cell_id,
                                'tma': expression_file.parent.name,
                                'tissue': expression_file.stem.replace('_expression_matrix', '')
                            }
                            
                            # Add expression values
                            for i, marker in enumerate(header[1:]):
                                row_data[marker] = expression_values[i]
        ...
        if not all_expression_data:
            ...
            markers_list = [str(marker) for marker in self.markers]
            empty_data = []
            empty_df = pd.DataFrame(empty_data, columns=['cell_id', 'tma', 'tissue'] + markers_list)
            return empty_df
...
def main():
    ...
    processor = UltimateCODEXProcessor(data_root, output_root)
