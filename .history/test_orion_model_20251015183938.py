    ...
    def __getitem__(self, idx: int):
        ...
        for c in range(self.C):
            or_log_resized[c] = np.array(
                T.ToPILImage()(or_log[..., c]).resize((ps, ps), resample=T.InterpolationMode.BILINEAR)
            )
...
def compute_metrics(pred_log, tgt_log):
    ...
    pred_lin = np.expm1(pred_np)
    tgt_lin = np.expm1(tgt_np)
    ...
    # Overall metrics
    metrics['overall_mse'] = float(mean_squared_error(tgt_lin.flatten(), pred_lin.flatten()))
    metrics['overall_mae'] = float(mean_absolute_error(tgt_lin.flatten(), pred_lin.flatten()))
