...
def robust_normalize_channel(channel, gamma=0.5, percentile_low=1, percentile_high=99.5):
    """
    Normalize a single fluorescence channel using percentile-based scaling and gamma correction.
    This mimics how actual fluorescence images are typically displayed.
    """
    # Get percentile-based range
    vmin, vmax = np.percentile(channel, [percentile_low, percentile_high])
    ...
    # Apply gamma correction for better visibility (typical in fluorescence microscopy)
...
def visualize_predictions(he_tensor, pred_log, tgt_log, basename, zoom, output_path, num_channels=6):
    """Create visualization comparing predictions to ground truth with proper fluorescence display."""
    he_img = denormalize_he_tensor(he_tensor)
    pred_np = pred_log.cpu().numpy()[0] if pred_log.ndim == 4 else pred_log.cpu().numpy()  # (20, H, W)
    tgt_np = tgt_log.cpu().numpy()[0] if tgt_log.ndim == 4 else tgt_log.cpu().numpy()    # (20, H, W)
    ...
    # Show first num_channels markers with difference map
    fig, axes = plt.subplots(nrows=num_channels, ncols=4, figsize=(12, num_channels * 2.5))
    if num_channels == 1:
    ...
    for c in range(num_channels):
        ...
        # Normalize each channel independently for better visibility
        tgt_vis = robust_normalize_channel(tgt_lin[c], gamma=0.5)
        pred_vis = robust_normalize_channel(pred_lin[c], gamma=0.5)
        
        # Column 0: H&E
        axes[c, 0].imshow(he_img)
        axes[c, 0].set_title(f"H&E\nMarker {c}", fontsize=8, fontweight='bold')
        axes[c, 0].axis('off')
        
        # Column 1: Ground Truth
        im_gt = axes[c, 1].imshow(tgt_vis, cmap=cmap, vmin=0, vmax=1)
        axes[c, 1].set_title(f"Ground Truth", fontsize=8, color=color)
        axes[c, 1].axis('off')
        plt.colorbar(im_gt, ax=axes[c, 1], fraction=0.046, pad=0.04)
        
        # Column 2: Prediction
        im_pred = axes[c, 2].imshow(pred_vis, cmap=cmap, vmin=0, vmax=1)
        axes[c, 2].set_title(f"Prediction", fontsize=8, color=color)
        axes[c, 2].axis('off')
        plt.colorbar(im_pred, ax=axes[c, 2], fraction=0.046, pad=0.04)
        
        # Column 3: Difference map
        diff = np.abs(tgt_vis - pred_vis)
        im_diff = axes[c, 3].imshow(diff, cmap='hot', vmin=0, vmax=1)
        axes[c, 3].set_title(f"Difference\n|GT - Pred|", fontsize=8)
        axes[c, 3].axis('off')
        plt.colorbar(im_diff, ax=axes[c, 3], fraction=0.046, pad=0.04)
    
    plt.suptitle(f"{basename} @ Zoom {zoom:.1f}x | Individual Channels", fontsize=12, y=0.998, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    plt.close()


def create_composite_overlay(channels, channel_indices, colors, gamma=0.5):
    """
    Create a composite fluorescence image by overlaying multiple channels with additive blending.
    This mimics how multi-channel fluorescence images are typically displayed.
    
    Args:
        channels: (C, H, W) array of channels
        channel_indices: List of channel indices to include
        colors: List of (R, G, B) tuples for each channel
        gamma: Gamma correction value
    
    Returns:
        (H, W, 3) RGB composite image
    """
    H, W = channels.shape[1:]
    composite = np.zeros((H, W, 3), dtype=np.float32)
    
    for idx, color in zip(channel_indices, colors):
        if idx >= channels.shape[0]:
            continue
        
        # Normalize channel with gamma correction
        ch_norm = robust_normalize_channel(channels[idx], gamma=gamma)
        
        # Add to composite with color (additive blending)
        for rgb_idx in range(3):
            composite[:, :, rgb_idx] += ch_norm * color[rgb_idx]
...
def visualize_composite_overlays(he_tensor, pred_log, tgt_log, basename, zoom, output_path):
    """
    Create composite overlay comparisons showing multiple markers combined.
    This shows GT vs Pred for different channel combinations.
    """
    he_img = denormalize_he_tensor(he_tensor)
    pred_np = pred_log.cpu().numpy()[0] if pred_log.ndim == 4 else pred_log.cpu().numpy()
    tgt_np = tgt_log.cpu().numpy()[0] if tgt_log.ndim == 4 else tgt_log.cpu().numpy()
    ...
    # Row 0: H&E and RGB composite (markers 0, 1, 2)
    axes[0, 0].imshow(he_img)
    axes[0, 0].set_title("H&E Input", fontsize=11, fontweight='bold')
    axes[0, 0].axis('off')
    
    rgb_gt = create_composite_overlay(tgt_lin, [0, 1, 2], 
                                      [(1, 0, 0), (0, 1, 0), (0, 0, 1)], gamma=0.5)
    axes[0, 1].imshow(rgb_gt)
    axes[0, 1].set_title("GT Composite\n(M0:Red, M1:Green, M2:Blue)", fontsize=10)
    axes[0, 1].axis('off')
    
    rgb_pred = create_composite_overlay(pred_lin, [0, 1, 2],
                                        [(1, 0, 0), (0, 1, 0), (0, 0, 1)], gamma=0.5)
    axes[0, 2].imshow(rgb_pred)
    axes[0, 2].set_title("Pred Composite\n(M0:Red, M1:Green, M2:Blue)", fontsize=10)
    axes[0, 2].axis('off')
    
    # Row 1: CMY composite (markers 3, 4, 5)
    cmy_gt = create_composite_overlay(tgt_lin, [3, 4, 5],
                                      [(0, 1, 1), (1, 0, 1), (1, 1, 0)], gamma=0.5)
    axes[1, 0].imshow(cmy_gt)
    axes[1, 0].set_title("GT Composite\n(M3:Cyan, M4:Magenta, M5:Yellow)", fontsize=10)
    axes[1, 0].axis('off')
    
    cmy_pred = create_composite_overlay(pred_lin, [3, 4, 5],
                                        [(0, 1, 1), (1, 0, 1), (1, 1, 0)], gamma=0.5)
    axes[1, 1].imshow(cmy_pred)
    axes[1, 1].set_title("Pred Composite\n(M3:Cyan, M4:Magenta, M5:Yellow)", fontsize=10)
    axes[1, 1].axis('off')
    
    # Difference for CMY
    diff_cmy = np.abs(cmy_gt - cmy_pred).mean(axis=2)
    im = axes[1, 2].imshow(diff_cmy, cmap='hot', vmin=0, vmax=1)
    axes[1, 2].set_title("Difference\n|GT - Pred|", fontsize=10)
    axes[1, 2].axis('off')
    plt.colorbar(im, ax=axes[1, 2], fraction=0.046, pad=0.04)
    
    # Row 2: Next set (markers 6, 7, 8)
    mix1_gt = create_composite_overlay(tgt_lin, [6, 7, 8],
                                       [(1, 0.5, 0), (0.5, 0, 1), (0, 0.8, 0.4)], gamma=0.5)
    axes[2, 0].imshow(mix1_gt)
    axes[2, 0].set_title("GT Composite\n(M6:Orange, M7:Purple, M8:Teal)", fontsize=10)
    axes[2, 0].axis('off')
    
    mix1_pred = create_composite_overlay(pred_lin, [6, 7, 8],
                                         [(1, 0.5, 0), (0.5, 0, 1), (0, 0.8, 0.4)], gamma=0.5)
    axes[2, 1].imshow(mix1_pred)
    axes[2, 1].set_title("Pred Composite\n(M6:Orange, M7:Purple, M8:Teal)", fontsize=10)
    axes[2, 1].axis('off')
    
    # Difference
    diff_mix1 = np.abs(mix1_gt - mix1_pred).mean(axis=2)
    im = axes[2, 2].imshow(diff_mix1, cmap='hot', vmin=0, vmax=1)
    axes[2, 2].set_title("Difference\n|GT - Pred|", fontsize=10)
    axes[2, 2].axis('off')
    plt.colorbar(im, ax=axes[2, 2], fraction=0.046, pad=0.04)
    
    # Row 3: All markers overlay (first 10)
    all_indices = list(range(10))
    all_colors = [FLUOR_COLORS[i] for i in range(10)]
    
    all_gt = create_composite_overlay(tgt_lin, all_indices, all_colors, gamma=0.4)
    axes[3, 0].imshow(all_gt)
    axes[3, 0].set_title("GT All Markers\n(M0-M9 Overlay)", fontsize=10, fontweight='bold')
    axes[3, 0].axis('off')
    
    all_pred = create_composite_overlay(pred_lin, all_indices, all_colors, gamma=0.4)
    axes[3, 1].imshow(all_pred)
    axes[3, 1].set_title("Pred All Markers\n(M0-M9 Overlay)", fontsize=10, fontweight='bold')
    axes[3, 1].axis('off')
    
    # Difference
    diff_all = np.abs(all_gt - all_pred).mean(axis=2)
    im = axes[3, 2].imshow(diff_all, cmap='hot', vmin=0, vmax=1)
    axes[3, 2].set_title("Difference\n|GT - Pred|", fontsize=10)
    axes[3, 2].axis('off')
    plt.colorbar(im, ax=axes[3, 2], fraction=0.046, pad=0.04)
    
    plt.suptitle(f"{basename} @ Zoom {zoom:.1f}x | Multi-Channel Composite Overlays", 
...
def visualize_all_markers_grid(he_tensor, pred_log, tgt_log, basename, zoom, output_path):
    """Create comprehensive grid showing all 20 markers with improved fluorescence display."""
    he_img = denormalize_he_tensor(he_tensor)
    pred_np = pred_log.cpu().numpy()[0] if pred_log.ndim == 4 else pred_log.cpu().numpy()
    tgt_np = tgt_log.cpu().numpy()[0] if tgt_log.ndim == 4 else tgt_log.cpu().numpy()
    ...
    # Create 5x5 grid (H&E + 20 markers, each showing GT vs Pred side by side)
    ...
    # Remaining cells: markers with improved normalization
    for c in range(20):
        row = (c + 1) // 5
        col = (c + 1) % 5
        ...
        # Normalize with proper fluorescence display techniques
        tgt_vis = robust_normalize_channel(tgt_lin[c], gamma=0.5)
        pred_vis = robust_normalize_channel(pred_lin[c], gamma=0.5)
        ...
        # Add divider line
        H, W = tgt_vis.shape
        ax.axvline(x=W-0.5, color='white', linewidth=2, alpha=0.8)
    ...
    for zoom in args.zoom_levels:
        ...
        for batch_idx, batch in enumerate(test_loader):
            ...
            if sample_count < args.num_visualizations:
                ...
                # Detailed visualization (first 6 markers with difference maps)
                ...
                # Composite overlays (multiple markers combined)
                vis_path_composite = zoom_dir / f"{basename}_y{info['y0'][0]}_x{info['x0'][0]}_composite.png"
                visualize_composite_overlays(he, pred_log, tgt_log, basename, zoom, vis_path_composite)
