...
def test_multiplex_patches(pairs_dir: str):
    ...
    sample_size = min(10, len(orion_files))
    success_count = 0
        ...
        if he_file.exists():
            try:
                ...
                if orion_patch.ndim == 3:
                    if orion_patch.shape[2] > 3:  # More than RGB channels
                        print(f"  ✅ SUCCESS: Multi-channel Orion with {orion_patch.shape[2]} channels!")
                        multi_channel_count += 1
                        success_count += 1
                    elif orion_patch.shape[2] == 1:
                        print(f"  ⚠️  SINGLE-CHANNEL: Only 1 channel found")
                        single_channel_count += 1
                    else:
                        print(f"  ⚠️  RGB-like: {orion_patch.shape[2]} channels (may be RGB)")
                elif orion_patch.ndim == 2:
                    print(f"  ⚠️  SINGLE-CHANNEL: 2D patch (no channel dimension)")
                    single_channel_count += 1
                else:
                    print(f"  ❓ UNKNOWN: Unusual shape {orion_patch.shape}")
                    
                # Check data range
                orion_min, orion_max = orion_patch.min(), orion_patch.max()
                print(f"  📊 Orion range: [{orion_min:.1f}, {orion_max:.1f}]")
                    
                # Validate H&E shape
                if he_patch.ndim == 3 and he_patch.shape[2] == 3:
                    print(f"  ✅ H&E has correct RGB shape")
                else:
                    print(f"  ⚠️  H&E has unexpected shape: {he_patch.shape}")
    ...
    print("=" * 50)
    print(f"🎯 SUMMARY:")
    print(f"  Multi-channel patches: {multi_channel_count}/{sample_size}")
    print(f"  Single-channel patches: {single_channel_count}/{sample_size}")
    print(f"  Expected for Orion: 20-channel patches like (256, 256, 20)")
    
    if multi_channel_count > 0:
        print("✅ SUCCESS: Multi-channel registration is working!")
        print("🎉 You should now be able to train on all Orion protein channels!")
        return True
    else:
        print("❌ FAILURE: No multi-channel patches found")
        print("💡 SOLUTION: Re-run the registration pipeline with the updated code")
        return False
