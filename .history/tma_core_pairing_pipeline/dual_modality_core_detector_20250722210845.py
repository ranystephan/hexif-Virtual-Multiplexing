...
using SpaceC tissue extraction with modality-specific parameter optimization.
"""
...
class CoreDetectionConfig:
    ...
    # H&E specific parameters
    he_lower_cutoff: float = 0.15
    he_upper_cutoff: float = 0.25
    
    # Orion specific parameters  
    orion_lower_cutoff: float = 0.10
    orion_upper_cutoff: float = 0.20
    dapi_channel: int = 0
    ...
    # Quality control
    min_core_area: int = 10000  # minimum area in pixels
    max_core_area: int = 1000000  # maximum area in pixels
...
class DualModalityCoreDetector:
    ...
    Uses SpaceC for robust tissue detection with modality-specific parameter optimization
    and intelligent spatial matching to pair corresponding cores.
    """
    ...
    def _detect_cores_orion(self, orion_path: str) -> Tuple[pd.DataFrame, Dict]:
        ...
        # Load Orion image and extract DAPI channel
        ...
        # Save DAPI channel as temporary image for SpaceC
        dapi_temp_path = self.temp_dir / "orion_dapi_temp.tif"
        imwrite(dapi_temp_path, dapi_channel)
        
        # Downscale DAPI channel
        resized_im = sp.hf.downscale_tissue(
            file_path=str(dapi_temp_path),
            downscale_factor=self.config.downscale_factor,
            padding=self.config.padding,
            output_dir=str(self.temp_dir)
        )
