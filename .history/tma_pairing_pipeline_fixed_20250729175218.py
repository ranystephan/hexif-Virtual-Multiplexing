#!/usr/bin/env python3
"""
tma_pairing_pipeline_fixed.py
-----------------------------
Fixed version with better default parameters for core detection.

python tma_pairing_pipeline_fixed.py \
...
# ---------------------------------------------------------------------------
# FIXED Detector with better default parameters
# ---------------------------------------------------------------------------

class ThumbnailCoreDetector:
    def __init__(self, thumb_size: int = 4096, min_core_area: int = 100):  # CHANGED: lowered from 2000 to 100
    ...
    def detect_cores(
        ...
        circularity_thresh: float = 0.05  # CHANGED: lowered from 0.3 to 0.05
    ) -> List[Dict]:
        ...
        logger.info(f"Filtering results: {area_filtered} by area, {moment_filtered} by moments, {circularity_filtered} by circularity")
...
# ---------------------------------------------------------------------------
# Main function with better parameter defaults
# ---------------------------------------------------------------------------
...
def main():
    ...
    parser.add_argument("--min_core_area", type=int, default=100)  # CHANGED: lowered default
    parser.add_argument("--circularity_thresh", type=float, default=0.05)  # CHANGED: added parameter
    ...
    # Detect H&E cores
    logger.info("Detecting H&E cores...")
    he_thumb, he_full = detector.load_thumbnail(args.he)
    he_core_objs = detector.detect_cores(he_thumb, brightfield=True, min_area=args.min_core_area, 
    ...
    # Detect Orion cores
    logger.info("Detecting Orion cores...")
    or_thumb, or_full = detector.load_thumbnail(args.orion, use_channel0=True)
    or_core_objs = detector.detect_cores(or_thumb, brightfield=False, min_area=args.min_core_area,
                                        circularity_thresh=args.circularity_thresh)
    or_scaled = detector.scale_cores(or_core_objs, or_thumb.shape, or_full)
    or_centres = np.array([c['center'] for c in or_scaled])

    logger.info(f"Detected {len(he_centres)} H&E cores and {len(or_centres)} Orion cores")
