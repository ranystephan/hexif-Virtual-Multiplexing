...
def main():
    ...
    # MAE
    axes[0, 2].plot(epochs_range, history['train_mae'], label='Train', alpha=0.8)
    axes[0, 2].plot(epochs_range, history['val_mae'], label='Validation', alpha=0.8)
    ...
    # SSIM
    axes[1, 0].plot(epochs_range, history['train_ssim'], label='Train', alpha=0.8)
    axes[1, 0].plot(epochs_range, history['val_ssim'], label='Validation', alpha=0.8)
    ...
    # Validation Loss with Best Model Marker
    ...
    logger.info(f"Total training time: {sum(history.get('epoch_times', [0])):.1f}s")
    logger.info(f"Average epoch time: {np.mean(history.get('epoch_times', [0])):.1f}s")
    
    # Log model files
    logger.info("Generated files:")
    logger.info(f"  - Best model: {output_path / 'best_model.pth'}")
    logger.info(f"  - Final model: {final_model_path}")
    logger.info(f"  - Training history: {history_csv_path}")
    logger.info(f"  - Training curves: {curves_path}")
    logger.info(f"  - Training config: {output_path / 'training_config.json'}")
    logger.info(f"  - Training logs: {output_path / 'logs' / 'training.log'}")
