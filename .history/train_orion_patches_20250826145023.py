...
class OrionPatchDataset(Dataset):
    ...
    def __init__(
        ...
        # Cache for loaded arrays (critical for performance)
        # Use process-local cache to avoid multiprocessing issues
        self._cache = {}
        self._cache_size = min(20, len(basenames))  # Smaller cache per process
        self._cache_order = []  # Track access order for LRU
        
        # Process ID to ensure cache isolation between workers
        import os
    ...
    def _load_pair(self, idx_core: int):
        # Check if we're in a different process (worker) - reset cache if so
        import os
        ...
        # Use LRU cache to avoid reloading files repeatedly while limiting memory
        if idx_core in self._cache:
            # Move to end (most recently used)
            self._cache_order.remove(idx_core)
            self._cache_order.append(idx_core)
            return self._cache[idx_core]
        
        # Load and process the arrays
        he = np.load(self.he_paths[idx_core]).astype(np.float32)     # (H,W,3)
        ori = np.load(self.ori_paths[idx_core]).astype(np.float32)   # (H,W,20) or (20,H,W)
        
        # Normalize HE
        if he.max() > 1.0:  # [0..255] → [0..1]
            he = he / 255.0
...
def train_one_epoch(model, loader, criterion, optimizer, device, scaler=None):
    ...
    for batch_idx, (he, ori) in enumerate(loader):
        ...
        if batch_idx % 100 == 0 and batch_idx > 0:
            logging.info(f"Processed {batch_idx}/{len(loader)} batches")
        he = he.to(device, non_blocking=True)
        ori = ori.to(device, non_blocking=True)
...
def make_loaders(args):
    ...
    # Reduce worker count to avoid deadlocks with large datasets
    # Further reduce for very large datasets
    total_patches = len(train_ds)
    if total_patches > 500000:  # > 500K patches
        safe_workers = min(args.num_workers, 4)  # Cap at 4 workers for huge datasets
        logging.info(f"Large dataset detected ({total_patches} patches), using max 4 workers")
    else:
        safe_workers = min(args.num_workers, 8)  # Cap at 8 workers
...
def main():
    ...
    p.add_argument("--patches_per_image", type=int, default=4096)
    p.add_argument("--patches_per_image_val", type=int, default=1024)
    ...
    p.add_argument("--grid_stride", type=int, default=128, help="stride for grid sampling; default=patch_size")
