...
def make_loaders(args):
    ...
    # TEMPORARY FIX: Disable multiprocessing to avoid worker deadlocks
    # This will be slower but should work reliably
    safe_workers = 0  # Single-threaded for now
    logging.info(f"Using single-threaded data loading to avoid worker issues ({len(train_ds)} patches)")
    ...
    logging.info(f"Using single-threaded data loading for both training and validation")
