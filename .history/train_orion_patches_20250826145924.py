...
class OrionPatchDataset(Dataset):
    ...
    def _load_pair(self, idx_core: int):
        # Simple on-demand loading without caching to avoid memory issues
        he = np.load(self.he_paths[idx_core], mmap_mode='r').astype(np.float32)     # (H,W,3)
        ori = np.load(self.ori_paths[idx_core], mmap_mode='r').astype(np.float32)   # (H,W,20) or (20,H,W)
