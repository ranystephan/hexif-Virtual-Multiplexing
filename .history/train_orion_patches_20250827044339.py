...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # But since our model outputs sigmoid, we need to handle this carefully
        # Convert back to logits for safe BCE computation
        eps = 1e-8
        pred_clamped = torch.clamp(pred, eps, 1 - eps)
        logits = torch.log(pred_clamped / (1 - pred_clamped))
        
        bce_loss = F.binary_cross_entropy_with_logits(logits, target, reduction='none')
        
        # Compute focal weight using original sigmoid output
