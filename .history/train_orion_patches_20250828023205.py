...
python train_orion_patches.py --resume_from best_model.pth ...

# Resume training from specific epoch checkpoint
python train_orion_patches.py --resume_from checkpoint_epoch_10.pth ...
...
def main():
    ...
    # Resume training
    p.add_argument("--resume_from", type=str, default=None, help="Path to checkpoint to resume from (e.g., 'best_model.pth' or 'checkpoint_epoch_10.pth')")
    ...
    if args.resume_from:
        ...
        if not checkpoint_path.is_absolute():
            checkpoint_path = outdir / checkpoint_path
        ...
        if checkpoint_path.exists():
            ...
            logging.info(f"Resumed from epoch {checkpoint['epoch']}, best val loss: {best_val:.4f}")
        else:
            logging.warning(f"Checkpoint not found: {checkpoint_path}. Starting from scratch.")
