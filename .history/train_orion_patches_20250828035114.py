...
python train_orion_patches.py \
  ...
  --output_dir orion_patches_model_aug28 \
  --epochs 30 --patch_size 256 --base_features 32 \
  --batch_size 6 --gradient_accumulation_steps 2 --use_amp \
  --patches_per_image 12 --patches_per_image_val 8 \
  --train_sampling random --val_sampling grid --grid_stride 128 \
  --num_workers 8
...
class OrionPatchDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # Ensure writable, contiguous arrays to avoid PyTorch read-only warnings
        if not he_patch.flags.writeable:
            he_patch = he_patch.copy()
        if not ori_patch.flags.writeable:
            ori_patch = ori_patch.copy()
        he_patch = np.ascontiguousarray(he_patch)
        ori_patch = np.ascontiguousarray(ori_patch)
