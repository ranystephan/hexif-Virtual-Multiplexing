...
class SpatialOrionDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # Faster normalization - use simple min-max instead of percentile-based
        C = or_crop.shape[2]
        or_scaled = np.zeros_like(or_crop, dtype=np.float32)
        for c in range(C):
            if self.sampling == "random":  # Fast normalization for training
                min_val, max_val = or_crop[..., c].min(), or_crop[..., c].max()
                if max_val > min_val:
                    or_scaled[..., c] = (or_crop[..., c] - min_val) / (max_val - min_val)
                else:
                    or_scaled[..., c] = or_crop[..., c]
            else:  # Robust normalization for validation
