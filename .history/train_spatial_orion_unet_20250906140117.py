...
# Fluorescence colormap utilities for visualization
FLUOR_COLORS = [
    (0.0, 0.5, 1.0),   # Blue
    (0.0, 1.0, 0.0),   # Green
    (1.0, 0.0, 0.0),   # Red
...
# Default marker names (can be customized)
DEFAULT_MARKER_NAMES = [f"Marker_{i:02d}" for i in range(20)]
...
def create_sanity_check_visualization(
    ...
    """Create comprehensive visualization comparing H&E, ground truth, and predictions."""
    ...
    # Denormalize H&E
    he_img = denormalize_he_tensor(he_tensor.squeeze(0))  # Remove batch dim
    
    # Convert predictions and targets to numpy
    pred_np = pred_tensor.squeeze(0).detach().cpu().numpy()  # (20, H, W)
    target_np = target_tensor.squeeze(0).detach().cpu().numpy()  # (20, H, W)
    
    num_markers = pred_np.shape[0]
    
    # Create comprehensive visualization: H&E + all markers (GT vs Pred)
    fig, axes = plt.subplots(nrows=num_markers, ncols=3, figsize=(9, num_markers * 3))
    if num_markers == 1:
        axes = axes.reshape(1, -1)
    
    for c in range(num_markers):
        marker_name = marker_names[c] if c < len(marker_names) else f'Marker_{c:02d}'
        cmap = FLUOR_CMAPS[c % len(FLUOR_CMAPS)]
        
        # Column 1: H&E
        ax = axes[c, 0]
        ax.imshow(he_img)
        ax.set_title(f"{marker_name} - H&E", fontsize=8)
        ax.axis('off')
        
        # Column 2: Ground Truth
        ax = axes[c, 1]
        im_gt = ax.imshow(target_np[c], cmap=cmap, vmin=0, vmax=1)
        ax.set_title("Ground Truth", fontsize=8)
        ax.axis('off')
        
        # Column 3: Prediction
        ax = axes[c, 2]
        im_pred = ax.imshow(pred_np[c], cmap=cmap, vmin=0, vmax=1)
        ax.set_title("Prediction", fontsize=8)
        ax.axis('off')
    
    plt.suptitle(f"Epoch {epoch} - Sanity Check{patch_info}", fontsize=12, y=0.995)
    plt.tight_layout()
    
    # Save the figure
    filename = f"epoch_{epoch:03d}_sanity_check.png"
    filepath = sanity_dir / filename
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close()
    
    logging.info(f"Sanity check visualization saved: {filepath}")
...
class SpatialOrionDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # Optimized preprocessing - skip expensive operations during training
        if self.noise_removal and self.sampling == "grid":  # Only for validation
            or_crop = remove_noise_cells(or_crop)

        # Use robust normalization for both training and validation
        # The simple min-max normalization might be too weak for learning
        C = or_crop.shape[2]
...
def get_center_patch_from_dataset(dataset: SpatialOrionDataset, core_idx: int = 0) -> Tuple[int, int]:
    """Get coordinates for a patch near the center of a core."""
    if core_idx >= len(dataset.basenames):
        core_idx = 0
    
    H, W = dataset.shapes[core_idx]
    ps = dataset.ps
    
    # Calculate center coordinates
    center_y = max(0, min(H - ps, H // 2 - ps // 2))
    center_x = max(0, min(W - ps, W // 2 - ps // 2))
    
    return center_y, center_x


def extract_center_patch_for_visualization(
    dataset: SpatialOrionDataset, 
    device: torch.device,
    core_idx: int = 0
) -> Tuple[torch.Tensor, torch.Tensor]:
    """Extract a center patch from a specific core for visualization."""
    center_y, center_x = get_center_patch_from_dataset(dataset, core_idx)
    
    # Load the core data
    he, orion = dataset._load_pair(core_idx)
    he = dataset._to_float01(he)
    orion = dataset._to_float01(orion)
    
    ps = dataset.ps
    he_crop = he[center_y:center_y+ps, center_x:center_x+ps, :].copy()
    or_crop = orion[center_y:center_y+ps, center_x:center_x+ps, :].copy()
    
    # Apply same preprocessing as training
    if dataset.noise_removal:
        or_crop = remove_noise_cells(or_crop)
    ...
    # Convert to tensors (use eval transforms)
    he_img = (he_crop * 255).astype(np.uint8)
    he_t = dataset.tf_eval(he_img)  # (3, ps, ps)
    target = torch.from_numpy(or_scaled.transpose(2, 0, 1))  # (C, ps, ps)
    
    # Add batch dimension and move to device
    he_t = he_t.unsqueeze(0).to(device)
...
class SpatialAwareLoss(nn.Module):
    """Anti-collapse loss function for spatial marker prediction."""
    def __init__(self, 
    ...
    def forward(self, pred, target, valid_mask=None, boundary_mask=None):
        ...
        # Anti-collapse: Encourage spatial variance (more stable version)
        if self.variance_weight > 0:
            # Compute spatial variance for each channel
            pred_flat = pred_masked.view(pred_masked.shape[0], pred_masked.shape[1], -1)
            pred_var = torch.var(pred_flat, dim=2).mean()
            
            # More stable variance penalty: penalize low variance without explosive gradients
            target_variance = 0.01  # Target minimum variance
            if pred_var < target_variance:
                variance_loss = (target_variance - pred_var) ** 2
            else:
                variance_loss = torch.tensor(0.0, device=pred.device)
...
def make_loaders(args, device):
    ...
    val_bs = args.val_batch_size * num_gpus if num_gpus > 1 else args.val_batch_size
...
def main():
    ...
    p.add_argument("--batch_size", type=int, default=32, help="Batch size per GPU (will be split across GPUs)")
    p.add_argument("--val_batch_size", type=int, default=16, help="Validation batch size per GPU")
    p.add_argument("--learning_rate", type=float, default=2e-4, help="Higher LR for faster convergence")
    p.add_argument("--weight_decay", type=float, default=1e-4, help="Higher weight decay for regularization")
    p.add_argument("--val_split", type=float, default=0.2)
    p.add_argument("--num_workers", type=int, default=16, help="Increased for multi-GPU")
    p.add_argument("--patch_size", type=int, default=384, help="Patch size (larger = more spatial context)")
    p.add_argument("--patches_per_image", type=int, default=32, help="Patches per image for training (reduced for speed)")
    p.add_argument("--patches_per_image_val", type=int, default=16, help="Patches per image for validation")
    p.add_argument("--grid_stride", type=int, default=112)
    p.add_argument("--base_features", type=int, default=32)
    p.add_argument("--use_amp", action="store_true", default=True, help="Use automatic mixed precision for speed")
    ...
    p.add_argument("--mse_weight", type=float, default=0.7, help="MSE weight (reduced)")
    p.add_argument("--l1_weight", type=float, default=0.5, help="L1 weight (increased for sparsity)")
    p.add_argument("--boundary_weight", type=float, default=0.2, help="Boundary weight (increased)")
    p.add_argument("--focal_weight", type=float, default=0.1, help="Focal weight for bright regions")
    p.add_argument("--variance_weight", type=float, default=0.2, help="Anti-collapse variance weight")
    p.add_argument("--sanity_check_freq", type=int, default=1, help="Frequency of sanity check visualizations (every N epochs)")
    p.add_argument("--marker_names", type=str, nargs='*', help="Custom marker names for visualization")
    ...
    if device.type == 'cuda':
        ...
        for i in range(num_gpus):
            ...
            logging.info(f"GPU {i}: {gpu_name} ({gpu_mem}GB)")
    ...
    # Prepare visualization data (extract center patch from first validation core)
    vis_he, vis_target = extract_center_patch_for_visualization(
        val_loader.dataset, device, core_idx=0
    )
    vis_basename = val_loader.dataset.basenames[0] if val_loader.dataset.basenames else "unknown"
    ...
    # Use spatial-aware loss with anti-collapse
    ...
    # More conservative learning rate schedule for stability
    sched = optim.lr_scheduler.ReduceLROnPlateau(opt, mode='min', factor=0.5, patience=5, verbose=True, min_lr=1e-7)
    ...
    # Create initial sanity check (epoch 0) to see model before training
    logging.info("Creating initial sanity check visualization...")
    net.eval()
    with torch.no_grad():
        if args.use_boundary_guidance:
            boundary_mask = torch.zeros(vis_he.shape[0], 1, vis_he.shape[2], vis_he.shape[3], 
                                       device=device, dtype=vis_he.dtype)
            vis_pred = net(vis_he, boundary_mask)
        else:
            vis_pred = net(vis_he)
        
        patch_info = f" ({vis_basename} center patch - INITIAL)"
        marker_names = args.marker_names if args.marker_names else None
        create_sanity_check_visualization(
            vis_he, vis_target, vis_pred, 0, outdir, 
            marker_names=marker_names, patch_info=patch_info
        )
    ...
    for epoch in range(1, args.epochs+1):
        ...
        # Create sanity check visualization based on frequency
        if epoch % args.sanity_check_freq == 0:
            net.eval()
            with torch.no_grad():
                if args.use_boundary_guidance:
                    # Create dummy boundary mask for visualization
                    boundary_mask = torch.zeros(vis_he.shape[0], 1, vis_he.shape[2], vis_he.shape[3], 
                                               device=device, dtype=vis_he.dtype)
                    vis_pred = net(vis_he, boundary_mask)
                else:
                ...
                patch_info = f" ({vis_basename} center patch)"
                marker_names = args.marker_names if args.marker_names else None
                create_sanity_check_visualization(
                    vis_he, vis_target, vis_pred, epoch, outdir, 
                    marker_names=marker_names, patch_info=patch_info
                )
        ...
        if val < best_val:
            ...
            patience_counter += 1
            logging.info(f"  → No improvement ({patience_counter}/{early_stopping_patience})")
            
        # Early stopping check
        if patience_counter >= early_stopping_patience:
            logging.info(f"Early stopping triggered after {epoch} epochs (no improvement for {early_stopping_patience} epochs)")
            break
