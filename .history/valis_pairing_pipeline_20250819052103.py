...
def main():
    ...
    # Create a temporary directory containing copies of the slides for registration
    with tempfile.TemporaryDirectory() as tmp_dir:
        tmp_dir = Path(tmp_dir)
        he_name = Path(args.he).name
        orion_name = Path(args.orion).name
        tmp_he_path = tmp_dir / he_name
        tmp_orion_path = tmp_dir / orion_name
        shutil.copy(args.he, tmp_he_path)
        shutil.copy(args.orion, tmp_orion_path)
        
        # Verify files were copied successfully
        if not tmp_he_path.exists():
            raise FileNotFoundError(f"Failed to copy H&E file to {tmp_he_path}")
        if not tmp_orion_path.exists():
            raise FileNotFoundError(f"Failed to copy Orion file to {tmp_orion_path}")
        
        # Start JVM for BioFormats
        slide_io.init_jvm(mem_gb=args.mem_gb)
        try:
            # Initialize VALIS registrar with full paths
            reg = registration.Valis(
                src_dir=str(tmp_dir),
                dst_dir=str(tmp_dir),
                img_list=[str(tmp_he_path), str(tmp_orion_path)],
                imgs_ordered=True,
                reference_img_f=str(tmp_he_path),
            ...
            logger.info("Registering slides with VALIS...")
            reg.register()
            # Retrieve slide objects
            he_slide = reg.get_slide(str(tmp_he_path))
            or_slide = reg.get_slide(str(tmp_orion_path))
