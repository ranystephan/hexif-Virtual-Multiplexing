...
def convert_ome_tiff_to_standard_tiff(input_path: str, output_path: str) -> bool:
    ...
    try:
        ...
        with tifffile.TiffFile(input_path) as tif:
            ...
            # Write as standard TIFF with compression, using simpler parameters to avoid format issues
            tifffile.imwrite(
                output_path, 
                data, 
                compression='zlib',
                planarconfig='contig',  # Ensure contiguous format
                # Remove photometric parameter that might cause issues with multi-channel data
            )
