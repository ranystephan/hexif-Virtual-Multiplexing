...
def detect_cores(
    ...
    # Enhanced preprocessing for fluorescence images
    if not brightfield:
        # Apply CLAHE for better contrast
        clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8))
        img = clahe.apply(img)
        # Additional smoothing for fluorescence
        blur = cv2.GaussianBlur(img, (7, 7), 0)
        # Try median filter to reduce noise
        blur = cv2.medianBlur(blur, 5)
    else:
        blur = cv2.GaussianBlur(img, (5, 5), 0)
    
    # Adaptive block size based on image dimensions
    bs = max(31, int(min(blur.shape) // 40) * 2 + 1)  # Slightly larger blocks
    ...
    if brightfield:
        ...
        # Larger kernel for fluorescence to connect fragmented cores
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (7, 7))
    
    binary = cv2.morphologyEx(binary, cv2.MORPH_OPEN, kernel)
    binary = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, kernel)
    
    # Additional closing for fluorescence to fill gaps
    if not brightfield:
    ...
    # Calculate area bounds based on typical TMA core sizes
    max_area = min_area * 50  # Reasonable upper bound
    ...
    for cnt in contours:
        ...
        if circularity < circularity_thresh:
            continue
        cores.append({'center': (cx, cy), 'area': area, 'circularity': circularity})
    
    # Sort by area (larger cores first) to prioritize well-defined cores
...
def main():
    ...
    parser.add_argument("--min_core_area", type=int, default=500, help="Minimum contour area on thumbnail (increased from 100)")
    parser.add_argument("--circularity_thresh", type=float, default=0.15, help="Minimum contour circularity (increased from 0.05 for better core detection)")
    parser.add_argument("--max_dist_factor", type=float, default=5.0, help="Maximum pairing distance factor (increased from 3.0 for more tolerance)")
    ...
    # Detect cores on Orion slide
    logger.info("Detecting cores on Orion thumbnail...")
    orion_thumb, orion_full_shape = load_thumbnail(args.orion, thumb_size=args.thumb_size, use_channel0=True)
    orion_cores = detect_cores(orion_thumb, brightfield=False, min_area=args.min_core_area, circularity_thresh=args.circularity_thresh)
    orion_centres = scale_centres(orion_cores, orion_thumb.shape, orion_full_shape)
    logger.info(f"Detected {len(orion_centres)} Orion cores")
    
    # Convert to numpy arrays
    he_centres_arr = np.array(he_centres, dtype=float) if he_centres else np.empty((0, 2))
    orion_centres_arr = np.array(orion_centres, dtype=float) if orion_centres else np.empty((0, 2))
    ...
    if len(he_centres_arr) > 0:
        ...
        warped_he_xy = he_slide.warp_xy_from_to(
            xy=he_centres_arr,
        ...
        logger.info(f"Warped {len(warped_he_xy)} H&E core coordinates to Orion space")
    else:
        warped_he_xy = np.empty((0, 2))
    
    # Debug output
    logger.info(f"H&E cores detected: {len(he_centres_arr)}")
    logger.info(f"Orion cores detected: {len(orion_centres_arr)}")
    logger.info(f"Warped H&E coordinates: {len(warped_he_xy)}")
    
    # Pair warped H&E coordinates with detected Orion coordinates
    pairs = pair_points(warped_he_xy, orion_centres_arr, max_dist_factor=args.max_dist_factor)
    logger.info(f"Created {len(pairs)} pairs using Hungarian assignment")
    # Extract and save patches
    csv_rows = []
    for idx, (warped_he_idx, or_idx) in enumerate(pairs, 1):
        # Use original H&E coordinates for H&E patch extraction
        he_center = he_centres_arr[warped_he_idx]
        # Use detected Orion coordinates for Orion patch extraction
        or_center = orion_centres_arr[or_idx]
        ...
        # Calculate distance between warped H&E coordinate and detected Orion coordinate
        warped_he_center = warped_he_xy[warped_he_idx]
        dist = float(np.linalg.norm(or_center - warped_he_center))
