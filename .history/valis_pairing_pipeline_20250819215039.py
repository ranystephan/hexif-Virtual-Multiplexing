...
def create_registration_overview(he_slide, or_slide, he_centres_arr, warped_or_xy, pairs, output_path: str, max_dim: int = 2048):
    ...
    """
    try:
        # Get thumbnails from both slides using VALIS slide reader methods
        try:
            he_thumb = he_slide.get_thumbnail(max_dim)
        except AttributeError:
            # Fallback method for VALIS slide objects
            he_thumb = he_slide.slide_reader.read_region(level=he_slide.slide_reader.get_best_level_for_downsample(max_dim/max(he_slide.slide_dimensions_wh)))
            
        try:
            or_thumb = or_slide.get_thumbnail(max_dim)
        except AttributeError:
            # Fallback method for VALIS slide objects
            or_thumb = or_slide.slide_reader.read_region(level=or_slide.slide_reader.get_best_level_for_downsample(max_dim/max(or_slide.slide_dimensions_wh)))
...
def pair_points(or_points: np.ndarray, he_points: np.ndarray, max_dist_factor: float = 3.0) -> List[Tuple[int, int]]:
    ...
    # Try multiple pairing strategies with different thresholds
    strategies = [
        ("strict", max_dist_factor * spacing),
        ("moderate", max_dist_factor * spacing * 2), 
        ("lenient", max_dist_factor * spacing * 5),
        ("very_lenient", np.percentile(dists, 10))  # Use 10th percentile as threshold
    ]
    
    best_pairs = []
    best_strategy = "none"
    
    for strategy_name, max_allowed_dist in strategies:
        logger.info(f"Trying {strategy_name} strategy with max distance: {max_allowed_dist:.1f}")
        ...
        logger.info(f"{strategy_name} strategy: {len(pairs)}/{len(row_ind)} pairs accepted")
        
        # Use the first strategy that produces reasonable results
        if len(pairs) >= min(10, len(valid_or_points) // 10):  # At least 10 pairs or 10% of cores
            best_pairs = pairs
            best_strategy = strategy_name
            break
        elif len(pairs) > len(best_pairs):
            best_pairs = pairs
            best_strategy = strategy_name
    
    logger.info(f"Selected {best_strategy} strategy with {len(best_pairs)} final pairs")
    
    # Log some statistics about the selected pairs
    if len(best_pairs) > 0:
        pair_distances = []
        for or_idx, he_idx in best_pairs:
            or_orig_idx = np.where(valid_or_mask)[0][or_idx] if or_idx < len(np.where(valid_or_mask)[0]) else or_idx
            he_orig_idx = np.where(valid_he_mask)[0][he_idx] if he_idx < len(np.where(valid_he_mask)[0]) else he_idx
            if or_orig_idx < len(or_points) and he_orig_idx < len(he_points):
                dist = np.linalg.norm(or_points[or_orig_idx] - he_points[he_orig_idx])
                pair_distances.append(dist)
        
        if pair_distances:
            logger.info(f"Pair distance statistics: min={min(pair_distances):.1f}, "
                       f"median={np.median(pair_distances):.1f}, max={max(pair_distances):.1f}")
...
def main():
    ...
    parser.add_argument("--max_dist_factor", type=float, default=5.0, help="Maximum pairing distance factor (increased default for better pairing)")
    parser.add_argument("--max_cores", type=int, default=500, help="Maximum number of cores to process (for memory management)")
    ...
    parser.add_argument("--auto_non_rigid", action="store_true", default=True, help="Automatically try non-rigid if rigid fails to produce pairs (default: True)")
    ...
    # Limit number of cores for memory management and processing efficiency
    if len(he_centres) > args.max_cores:
        logger.warning(f"Detected {len(he_centres)} cores, limiting to {args.max_cores} largest cores for processing")
        # Sort cores by area (largest first) and take the top N
        he_cores_sorted = sorted(he_cores, key=lambda x: x['area'], reverse=True)[:args.max_cores]
        he_centres = [core['center'] for core in he_cores_sorted]
        he_centres = scale_centres([{'center': center} for center in he_centres], he_thumb.shape, he_full_shape)
        logger.info(f"Processing {len(he_centres)} largest cores")
    ...
    # If no pairs found and auto_non_rigid is enabled, try non-rigid registration
    if len(pairs) == 0 and args.auto_non_rigid and not args.non_rigid:
        logger.warning("No pairs found with rigid registration. Attempting non-rigid registration...")
        try:
            ...
            # Check for valid non-rigid warped coordinates
            valid_mask = np.isfinite(warped_or_xy_nonrigid).all(axis=1)
            valid_count = np.sum(valid_mask)
            logger.info(f"Non-rigid warping: {valid_count}/{len(warped_or_xy_nonrigid)} valid coordinates")
            
            if valid_count > 0:
                # Try pairing with non-rigid coordinates
                pairs_nonrigid = pair_points(warped_or_xy_nonrigid, he_centres_arr, max_dist_factor=args.max_dist_factor)
                logger.info(f"Non-rigid registration produced {len(pairs_nonrigid)} pairs")
                
                if len(pairs_nonrigid) > len(pairs):
                    logger.info("Non-rigid registration improved results, using non-rigid coordinates")
                    pairs = pairs_nonrigid
                    warped_or_xy = warped_or_xy_nonrigid
                    
                    # Update overview with non-rigid results
                    if args.create_overview:
                        overview_nonrigid_path = registration_qc_dir / "registration_overview_nonrigid.png"
                        create_registration_overview(he_slide, or_slide, he_centres_arr, warped_or_xy, pairs, str(overview_nonrigid_path))
                
        except Exception as e:
            logger.warning(f"Non-rigid registration failed: {e}")
