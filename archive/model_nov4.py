...
torchrun --nproc_per_node=4 model_nov4.py \
  --pairs_dir core_patches_npy \
  --output_dir runs/nov5/focal_l1_plateau \
  --epochs 80 \
  --batch_size 12 \
  --val_batch_size 10 \
  --patch_size 224 \
  ...
  --use_focal_presence \
  --w_presence 0.5 \
  --channel_stats_json output/data_summary_nov4/channel_summary.json \
  --sanity_every 1 \
  --num_workers 6
...
def main():
    ...
    p.add_argument("--loss_type", type=str, default="l2", choices=["l1", "l2"], help="Main regression loss type.")
    p.add_argument("--w_center", type=float, default=1.0, help="Weight for the primary center-window loss.")
    ...
    p.add_argument("--w_presence", type=float, default=0.25,
    ...
    p.add_argument("--use_focal_presence", action="store_true", default=False,
                   help="Use Focal Loss for the presence term.")
    p.add_argument("--focal_gamma", type=float, default=2.0, help="Gamma for Focal Loss.")
    p.add_argument("--focal_alpha", type=float, default=0.25, help="Alpha for Focal Loss.")
    p.add_argument("--scheduler", type=str, default="cosine", choices=["cosine", "plateau"], help="LR scheduler type.")
    args = p.parse_args()
    ...
    if args.scheduler == "plateau":
        sched = optim.lr_scheduler.ReduceLROnPlateau(opt, mode='min', factor=0.2, patience=5)
        if is_main_process(): logging.info("Using ReduceLROnPlateau scheduler.")
        ...
        if is_main_process(): logging.info(f"Using Cosine scheduler with {args.warmup_epochs} warmup epochs.")
    else:
        sched = optim.lr_scheduler.CosineAnnealingLR(opt, T_max=args.epochs, eta_min=1e-6)
        if is_main_process(): logging.info("Using Cosine scheduler.")
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if is_main_process():
            ...
            if is_valid and val < best_val:
                # Save the first valid model, then only save on epoch >= 2 if it's better.
