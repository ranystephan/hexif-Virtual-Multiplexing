...
\author*[2]{\fnm{Andrew} \sur{Gentles}}\email{andrewg@stanford.edu}

\author[2]{\fnm{Mohamed} \sur{Kouchak}}\email{mohamedk@stanford.edu}

\author[2]{\fnm{Magdalena} \sur{Matusiak}}\email{mmatusiak@stanford.edu}
...
\textbf{HistoPlexer}, developed by Andani and colleagues \cite{Andani2025}, represents a significant advance in multi-marker prediction from histopathology images. Their approach employs a conditional generative model to predict multiplex immunofluorescence patterns, demonstrating feasibility of comprehensive virtual multiplexing. Our work builds on this foundation while introducing several methodological innovations including channel-aware sampling and multi-component loss design.
...
The Orion multiplex imaging platform enables simultaneous detection of 20 protein markers through cyclic immunofluorescence with spectrally-resolved ArgoFluor detection \cite{RareCyte2020}. Unlike sequential staining approaches that require tissue stripping between cycles, Orion employs spectral unmixing algorithms to resolve overlapping fluorophore emissions, enabling single-round acquisition of the complete marker panel.
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{diagrams/pipeline_diagram.png}
\caption{\textbf{HExIF Computational Pipeline.} (A) Data Preparation \& Preprocessing: Paired H\&E and Orion Whole Slide Images (WSIs) are spatially registered using VALIS. Tissue cores are detected and extracted to generate aligned training patches. (B) Deep Learning Model: A Swin-UNet architecture processes H\&E patches to predict 20-channel multiplex immunofluorescence maps. The model is trained using a composite loss function ($L_{total}$) incorporating center-masked reconstruction ($L_{center}$), structural similarity ($L_{SSIM}$), and coverage ($L_{cov}$) terms. (C) Inference: The trained model is applied to unseen H\&E slides in a sliding window fashion to generate whole-slide virtual multiplex immunofluorescence images.}\label{fig:pipeline}
\end{figure}
